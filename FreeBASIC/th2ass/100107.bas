...
loadimage "./data/gfx/tiles.abn",0,floor1.i()
loadimage "./data/gfx/tiles.abn",1,floor2.i()
loadimage "./data/gfx/tiles.abn",2,ladder.i()
loadimage "./data/gfx/tiles.abn",3,floor3.i()
loadimage "./data/gfx/tiles.abn",4,floor4.i()
loadimage "./data/gfx/tiles.abn",5,bg1.i()
loadimage "./data/gfx/tiles.abn",6,bg2.i()
loadimage "./data/gfx/tiles.abn",7,floor5.i()
loadimage "./data/gfx/tiles.abn",8,rocks.i()
loadimage "./data/gfx/tiles.abn",9,lava.i()
loadimage "./data/gfx/tiles.abn",10,pizza.i()
loadimage "./data/gfx/tiles.abn",11,coin.i()
loadimage2 "./data/gfx/tiles2.abn",0,lf.s()
loadimage2 "./data/gfx/tiles2.abn",1,hlt1.s()
loadimage2 "./data/gfx/tiles2.abn",2,hlt2.s()
loadgmap "./data/gmap.abm"

char.lf=3:char.hlt=10:char.d=1
gx=3:gy=3
'char.x=16:char.y=16
...
do
    ...
    next snkcou
    if char.sy < 0 then if colmap(int((char.x+1)/16),int(char.y/16))=1 or colmap(int((char.x+14)/16),int(char.y/16))=1 then char.sy=0:char.y=int(char.y/16+1)*16
    ...
    if char.sx<>0 or char.sy<>0 then if colmap(int((char.x+4)/16),int((char.y/16)+2))=1 or colmap(int((char.x+11)/16),int((char.y/16)+2))=1 then:char.sy=0:char.st=1:char.y=int(char.y/16)*16:else:char.st=0:endif
    ...
    select case char.y
    case is < 5:gy-=1:char.y=240-32:goto loadup
    case is > 223:gy+=1:char.y=5:goto loadup
    end select
...
sub drawtile (tile() as ubyte, x as double, y as double, w as double, h as double, d as integer)
	...
	if d = -1 then
		for i = 0 to 15
			put (x+i*scw,y), tile, ((15-i)*scw,0)-step (scw-1,16*scw), trans
		next
	else
		put (x,y), tile, trans
	end if
...
sub drawtile_gm1 (tile() as unsigned integer, x as double, y as double, w as double, h as double, d as integer)
	dim as integer n1,n2
	dim as unsigned integer ctrans
	ctrans = rgb(255,0,255)
	if d=1 then
		for n1 = 0 TO 15:for n2 = 0 to 15
			if tile(n2,n1) <> ctrans then
				line (x+(n2*w),y+(n1*h))-(x+(n2*w)+w-1,y+(n1*h)+h-1),tile(n2,n1),BF
			endif
		next n2:next n1
	elseif d=-1 then
		for n1 = 0 TO 15:for n2 = 0 to 15
			if tile(n2,n1) <> ctrans then
				line (x+((-n2+15)*w),y+(n1*h))-(x+((-n2+15)*w)+w-1,y+(n1*h)+h-1),tile(n2,n1),BF
			endif
		next n2:next n1
	endif
end sub
...
sub drawtile2_gm1 (tile() as unsigned integer, x as double, y as double, w as double, h as double)
	dim as integer n1,n2
	dim as unsigned integer ctrans
	ctrans = rgb(255,0,255)
...
sub drawtile_old (tile() as ubyte, x as double, y as double, w as double, h as double, d as integer)
    dim as integer n1,n2
    if d=1 then
        for n1 = 0 TO 15:for n2 = 0 to 15
...
sub drawtile2_old (tile() as ubyte, x as double, y as double, w as double, h as double)
    dim as integer n1,n2
    for n1 = 0 TO 7:for n2 = 0 to 7
        if not (tile(n2,n1,0) = 255 and tile(n2,n1,1) = 0 and tile(n2,n1,2) = 255) then
        line (x+(n2*w),y+(n1*h))-(x+(n2*w)+w-1,y+(n1*h)+h-1),rgb(tile(n2,n1,0),tile(n2,n1,1),tile(n2,n1,2)),BF:endif
...
sub loadimage (file as string, b as integer, tile() as ubyte)
    ...
    dim as string a
    z = b*768
    dim ttmp(0 to 15, 0 to 15, 0 to 2) as ubyte
    open file for binary access read as #1
        ...
        for y = 0 to 15:for x = 0 to 15:z+=1:get #1,z,ttmp(x,y,0):next x:next y
        for y = 0 to 15:for x = 0 to 15:z+=1:get #1,z,ttmp(x,y,1):next x:next y
        for y = 0 to 15:for x = 0 to 15:z+=1:get #1,z,ttmp(x,y,2):next x:next y
    close #1
    screenlock
    for y = 0 to 15:for x = 0 to 15
      line (x*scw,y*sch)-(x*scw+scw-1,y*sch+sch-1), rgb(ttmp(x,y,0),ttmp(x,y,1),ttmp(x,y,2)),bf
    next x:next y
    get (0,0)-(scw*16-1,sch*16-1), tile
    screenunlock
end sub
...
sub loadimage2 (file as string, b as integer, tile() as ubyte)
    dim as integer x, y, z=0
    z = b*192
    dim ttmp2(0 to 7, 0 to 7, 0 to 2) as ubyte
    open file for binary access read as #1
        if eof(1) then
            print "Error loading file "+file
            print "Press any key to terminate"
        ...
        for y = 0 to 7:for x = 0 to 7:z+=1:get #1,z,ttmp2(x,y,0):next x:next y
        for y = 0 to 7:for x = 0 to 7:z+=1:get #1,z,ttmp2(x,y,1):next x:next y
        for y = 0 to 7:for x = 0 to 7:z+=1:get #1,z,ttmp2(x,y,2):next x:next y
    close #1
    screenlock
    for y = 0 to 7:for x = 0 to 7
      line (x*scw,y*sch)-(x*scw+scw-1,y*sch+sch-1), rgb(ttmp2(x,y,0),ttmp2(x,y,1),ttmp2(x,y,2)),bf
    next x:next y
    get (0,0)-(scw*8-1,sch*8-1), tile
    screenunlock
end sub

sub loadimage_gm1 (file as string, b as integer, tile() as unsigned integer)
    dim as integer x, y, z=0
    z = b*768
    dim ttmp(0 to 15, 0 to 15, 0 to 2) as ubyte
    open file for binary access read as #1
    ...
    close #1
    for y = 0 to 15:for x = 0 to 15
      tile(x,y) = rgb(ttmp(x,y,0),ttmp(x,y,1),ttmp(x,y,2))
    next x:next y
end sub

sub loadimage2_gm1 (file as string, b as integer, tile() as unsigned integer)
    dim as integer x, y, z=0
    z = b*192
    dim ttmp2(0 to 7, 0 to 7, 0 to 2) as ubyte
    open file for binary access read as #1
    ...
    close #1
    for y = 0 to 7:for x = 0 to 7
      tile(x,y) = rgb(ttmp2(x,y,0),ttmp2(x,y,1),ttmp2(x,y,2))
    next x:next y
end sub
